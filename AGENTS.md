...
- Unless the user explicitly requests a local-only or non-public result, after an approved human review of a release-worthy Android keyboard change, build and publish its direct APK, then synchronize and publish the product page, manual, and interactive mock without asking again solely for those routine release steps. This default starts only after the applicable PDH human review and does not replace the explicit `PDH-human-review` approval required to close the ticket.

...
現行の公開準備版は **v0.15.15**（versionCode 31）です。配布用の未圧縮APK名は `gesture-ime-v0.15.15.apk` です。
...
日本語・テンキー左上の☺で絵文字レイヤーを開きます。候補欄の位置にAndroidX Emoji Pickerのカテゴリicon行が表示され、開くたびにRecentと一覧先頭へ戻ります。カテゴリiconはすべて固定48dp幅です。左端はかなレイヤーと同じ幅で、上からあ、#!、19、AZの切替列です。右側は最下段まで4行7列の縦スクロール一覧で、絵文字レイヤーに削除キーはありません。Recentは端末内で新しい順・重複なしに最大100件保存し、101件目では最も古い1件を除きます。private入力欄ではRecentを表示・保存しません。記号2行目の]右0.5wは⌫で、tap削除・下フリックEscです。

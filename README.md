...
現行の公開準備版は **v0.13.0**（versionCode 14）です。配布用の未圧縮APK名は `gesture-ime-v0.13.0.apk` です。
...
日本語・テンキー左上の☺で絵文字レイヤーを開きます。候補欄の位置にAndroidX Emoji Pickerのカテゴリicon行が表示され、先頭のRecentを含むカテゴリから3行の縦スクロール一覧を選べます。Recentは端末内で新しい順・重複なしに最大24件保存し、最下行のレイヤー切替と削除は固定です。private入力欄ではRecentを表示・保存しません。
...
Android 12以降では、左下のレイヤーキーを左へフリックすると、端末内日本語モデルが利用できる端末に限り音声入力レイヤーで認識を開始します。認識中はキャンセルキーの右に「認識中」と表示します。最終候補を確定すると同じ入力欄へ1回だけ入力し、音声入力レイヤーのまま直ちに次の認識を開始します。キャンセルまたはレイヤー・入力欄の切替まで繰り返せます。マイク権限はSetup画面からユーザー操作で許可します。端末内モデルがない場合は非対応と表示し、ネットワーク認識へ自動で切り替えません。

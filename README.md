...
Android 12以降では、左下のレイヤーキーを左へフリックすると、端末内日本語モデルが利用できる端末に限り音声入力レイヤーで認識を開始します。最下段は6等分で、キャンセル、非操作の「認識中」、句読点、Space、削除、Enterの順です。句読点はtapで「、」、左で「。」、上で「？」、右で「！」、下で「、」を直接確定し、Mozcの読みには加えません。表示中の「、。？！」は全てフリックでも入力できます。削除はtapだけで直前の1文字を消し、SpaceとEnterは他レイヤーと同じフリックを使います。これらの編集操作中も認識・音声候補を維持します。最終候補は通常の横候補欄ではなく固定された音声パネルに1行ずつ縦に表示し、似た候補では異なる文字だけを青い太字で示します。最終結果がないときは、最後の有効な途中結果を選択可能な候補として表示します。候補を確定すると同じ入力欄へ1回だけ入力し、一覧を消して音声入力レイヤーのまま直ちに次の認識を開始します。無言のまま端末内認識が時間切れになってもエラー表示へ移らず、自動的に認識を再開して次の発話を待ちます。キャンセルまたはレイヤー・入力欄の切替まで繰り返せます。マイク権限はSetup画面からユーザー操作で許可します。端末内モデルがない場合と無音以外の認識エラーはタップできないテキストで表示し、ネットワーク認識への切替や自動再試行はしません。

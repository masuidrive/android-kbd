...
現行の公開準備版は **v0.15.14**（versionCode 30）です。配布用の未圧縮APK名は `gesture-ime-v0.15.14.apk` です。
...
日本語・テンキー左上の☺で絵文字レイヤーを開きます。候補欄の位置にAndroidX Emoji Pickerのカテゴリicon行が表示され、すべて固定48dp幅です。先頭のRecentを含むカテゴリから、左端の固定layer railを除く3行7列の縦スクロール一覧を選べます。railは日本語と同じ順序で☺、#!、19、AZを並べます。Recentは端末内で新しい順・重複なしに最大100件保存し、101件目では最も古い1件を除きます。Recentは通常の絵文字gridだけに表示し、左端のlayer railと最下段右端の削除は固定です。private入力欄ではRecentを表示・保存しません。

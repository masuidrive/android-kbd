...
現行の公開準備版は **v0.14.0**（versionCode 15）です。配布用の未圧縮APK名は `gesture-ime-v0.14.0.apk` です。
...
Android 12以降では、左下のレイヤーキーを左へフリックすると、端末内日本語モデルが利用できる端末に限り音声入力レイヤーで認識を開始します。認識中はキャンセルキーの右に「認識中」と表示します。最終候補は通常の横候補欄ではなく固定された音声パネルに1行ずつ縦に表示します。最終結果がないときは、最後の有効な途中結果を選択可能な候補として表示します。候補を確定すると同じ入力欄へ1回だけ入力し、一覧を消して音声入力レイヤーのまま直ちに次の認識を開始します。キャンセルまたはレイヤー・入力欄の切替まで繰り返せます。マイク権限はSetup画面からユーザー操作で許可します。端末内モデルがない場合と認識エラーはタップできないテキストで表示し、ネットワーク認識への切替や自動再試行はしません。

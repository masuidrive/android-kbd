...
音声入力レイヤーは候補が空、権限未許可、端末非対応の状態でも最下段のキャンセル・状態・句読点・Space・削除・Enterを露出し、キャンセルで直前の文字レイヤーへ戻れます。Spaceフリックは通常欄では選択位置を直接変更し、抽出テキストを返さない、または選択変更を拒否する非private入力先だけDPAD矢印へ自動fallbackします。成功応答だけ返して移動を無視するターミナルには、設定の「ターミナル向けカーソル操作」でDPAD送信を強制できます。

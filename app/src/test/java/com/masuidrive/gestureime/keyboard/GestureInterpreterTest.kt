...
class GestureInterpreterTest {
    ...
    @Test fun `pointer thresholds select high standard and low distances without changing trackpad units`() {
        ...
        listOf(
            ...
            assertEquals(
                ...
                GestureUpdate.Selection(Direction.RIGHT),
                subject.move(pointerId, distance, 0f),
            )
        }

        subject.start(10, 0f, 0f, trackpad = true, pointerThresholds = FlickSensitivity.LOW.thresholds())
    ...
    @Test fun `shorter sensitivity thresholds keep center return and axis lock ordered`() {
        ...
        listOf(
            ...
            subject.start(pointerId, 0f, 0f, verticalOnly = true, pointerThresholds = sensitivity.thresholds())
            assertNull(subject.move(pointerId, 0f, -selection + .01f))
            assertEquals(GestureUpdate.Selection(Direction.UP), subject.move(pointerId, 0f, -selection))
            assertNull(subject.move(pointerId, 0f, -centerReturn - .01f))
            assertEquals(GestureUpdate.Selection(Direction.CENTER), subject.move(pointerId, 0f, -centerReturn))
        }
    }

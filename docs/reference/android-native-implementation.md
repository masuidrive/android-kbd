...
日本語・テンキーレイヤーは左上の`☺`から絵文字レイヤーを開く。絵文字レイヤーはrecent最大8件とAPK内catalogを空行なしで続ける上3行・8列の縦scroll viewportにする。recentが空ならcatalog先頭24件を表示し、最下行は既存layer flickと削除を固定する。recentは端末内で新しい順・重複なしに保存する。日本語とテンキーの空白キーは`Space`とだけ表示するが、フリックで4方向カーソル移動へ入る。トラックパッドは動き始めの主方向から横軸または縦軸を選び、指を離すまで固定するため斜め移動はしない。

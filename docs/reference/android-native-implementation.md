...
日本語・テンキーレイヤーは左上の`☺`から絵文字レイヤーを開く。絵文字レイヤーはrecent最大8件の1行と16件ずつの一覧2行を8列に揃え、最下行の既存layer flick、前頁、頁表示、次頁、削除で4行内に収める。recentは端末内で新しい順・重複なしに保存し、空枠は空のままにする。日本語とテンキーの空白キーは`Space`とだけ表示するが、フリックで4方向カーソル移動へ入る。トラックパッドは動き始めの主方向から横軸または縦軸を選び、指を離すまで固定するため斜め移動はしない。

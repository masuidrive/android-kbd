...
日本語レイヤーは全行5列に揃える。右端のEnterキーは3行目から4行目にまたがる縦長キーとする。日本語レイヤーの左列は上から十字配置の4矢印、`#!`、`19`、`AZ` とし、`#` で記号レイヤーへ移動する。テンキーレイヤーでも4矢印、`#!`、`あん`、`AZ` を並べる。テンキーレイヤーは `1 2 3 / 4 5 6 / 7 8 9 / - 0 .` とし、`-` は中央が `-`、左が `+`、上が `/`、右が `*`、下が `,` の5方向フリックにする。日本語とテンキーの空白キーは`Space`とだけ表示するが、フリックで4方向カーソル移動へ入る。カーソルレイヤーの左列は上から `あん`、`#!`、`19`、`AZ` とし、中央のSpaceはQWERTYと同じ4方向トラックパッドとして動作する。トラックパッドは動き始めの主方向から横軸または縦軸を選び、指を離すまで固定するため斜め移動はしない。
...
日本語レイヤーではキーの上に候補列を常時確保する。未入力時は説明文、読みがある時は変換候補を横並びで表示する。候補は選択中だけアクセント色にする。
...
Fold7外画面を狭い基準、内画面を広い基準として、幅からキー単位 `w` を計算する。内画面でもキー数とジェスチャーは変えず、キーの高さと左右余白を増やす。ヒンジやセーフ領域は `WindowInsets` を使って避ける。

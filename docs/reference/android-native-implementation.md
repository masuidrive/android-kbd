...
画面と文書ではレイヤー名を「日本語」「テンキー」「絵文字」「QWERTY」「記号」「音声」に統一する。`language` と `layer` は実装上の内部状態名として扱う。
...
| 2 | `C/A(0.5w) a…l .(0.5w)` | C/A、`^ _`、backslash、vertical bar、`~`、波括弧、角括弧、BS |
| 3 | `?} z x c v b n m , BS` | AZ、Esc、backtick、`! ? ; < > -`、Tab（右端） |
| 4 | `あん Space Enter` | `あん Space Enter` |

`C/A` は記号の副ラベルと同じ高さになるよう、上側の `C` と下側の `A` に少し間を空けて固定する。上スワイプで Alt、下スワイプで Ctrl を次の1キーへ適用する。方向選択中と選択後は上下ラベルを消して、中央に選ばれた `A` または `C` を表示する。選択後のタップは修飾状態を解除する。未選択の`C/A`はタップで発火せず、上スワイプでAlt、下スワイプでCtrlを選ぶ。`BS`はタップで削除、下フリックでEsc、左・上・右の未割当方向では削除タップとして扱う。QWERTYレイヤーの記号切替は中央の `?` と右後ろの淡い `}` を重ねた `?}` キーで記号へ入り、記号レイヤーでは中央の `A` と右後ろの淡い `Z` を重ねた `AZ` キーでQWERTYへ戻る。どちらも暗い背景のモードキーとする。記号レイヤーは ASCII 文字だけを収録する。

4行目のレイヤー切替キーはQWERTY形式では`あん`、テンキー形式では`AZ`とする。タップ先はキーごとの表示を維持する。文字レイヤーの左下キーでは左=音声入力、上=日本語、右=QWERTY、下=テンキーへ移動する。音声入力レイヤーでは中央=キャンセル、上=日本語、右=QWERTY、下=テンキーへ移動する。認識中だけキャンセルの右側に非操作の「認識中」を出し、最終候補の確定後もキャンセルまたはレイヤー切替まで同じ入力欄で次の認識を開始する。方向選択中は元の補助文字を消し、キー中央に移動先のレイヤー名だけを表示する。`?}`、`あん`、`AZ`、`19` の補助文字は主ラベルの約75%のサイズで揃える。変換中でない`Enter`は待機中に上側へ小さく`C-j`、中央に`Enter`、下側へ小さく`paste`を離して表示する。上スワイプ中は下側の`paste`を中央へ移動・拡大し、離すとクリップボード内容を確定する。下スワイプ中は中央に`C-j`を表示し、離すとCtrl+Jを送る。QWERTYの`Space`は上側に補助表示 `←↓↑→` を置き、操作中も同じ記号列を初期表示に使う。

日本語・テンキーレイヤーは左上の`☺`から絵文字レイヤーを開く。開くたびにAndroidX Emoji Pickerの横scrollカテゴリicon headerを候補欄の50dp位置でRecent（adapter position 0）へ戻し、カテゴリから一覧へ移動する。header下は8dpのgapと、かなレイヤー左端列と同じ幅の固定rail（上から`あ`、`?}`、`19`、`AZ`）の右にある最下段までの4行・7列縦scroll gridである。絵文字レイヤーに削除キーは置かない。recentは端末内で新しい順・重複なし最大100件に保存し、private欄ではRecentへ表示・保存しない。日本語とテンキーの空白キーは`Space`とだけ表示するが、フリックで4方向カーソル移動へ入る。テンキーの`.`はtapで`.`、左フリックで`,`、右フリックで`=`とし、未割当の上・下では`.`を入力する。トラックパッドは動き始めの主方向から横軸または縦軸を選び、指を離すまで固定するため斜め移動はしない。
...
| 用途 | 値 | 動作 |
|---|---:|---|
| 軸ロック | 高い11 / 標準15 / 低い19 dp | 縦・横の優勢方向を確定 |
| 選択開始 | 高い16 / 標準24 / 低い32 dp | フリック先を選択 |
| 復帰ヒステリシス | 高い9 / 標準13 / 低い17 dp | 選択後の細かな揺れを抑える |
| レイヤー移動 | 高い16 / 標準24 / 低い32 dp | あんの上・下、Enterの下を確定 |
| spaceトラックパッド開始 | 10 px | 長押しを待たずカーソル移動へ入る |
| 水平カーソル1文字 | 8 px | 書記素単位で左右移動 |
| 垂直カーソル1行 | 24 px | 見た目の折返し行に沿って上下移動 |
| 長押し | 450 ms | 英字アクセント候補を表示 |
| 連続削除開始 | 420 ms | 以後65 ms間隔で繰り返す |

端末の物理ピクセルではなく、Android側では `dp` と実機テストから閾値を調整する。音声・絵文字は旧標準の固定値（軸固定12dp・選択18dp・復帰10dp）を使う。開始後に方向を変えた場合は現在位置から再判定し、指が中心へ戻ったら通常入力へ戻す。ACTION_UPの最終座標も再判定してから確定する。`ACTION_CANCEL`、フォーカス喪失、画面回転では入力を確定せず全ポインターを破棄する。

## フリックラベルのアニメーション

英字キーには中央の主ラベルと上端の副ラベルを別レイヤーで描く。通常時の副ラベルは主ラベルより小さく、文字色を一段落として、キーを見た瞬間に主入力と区別できるようにする。

下フリックが設定感度の選択開始距離を越えたら、副ラベルを90msの `ease` で下へ13px移動し、1.7倍へ拡大する。同時に主ラベルを下へ22px移動し、透明度を0へ落とす。中心へ設定感度の復帰距離以内に戻ったら逆再生する。上スワイプでは主ラベルを上へ3px移動し、副ラベルを透明にして、大文字のプレビューを表示する。
...
- 各かなキーの中央・左・上・右・下と、各感度の選択開始距離未満の揺れを検証する。
- 全英字のタップ、上スワイプ大文字、下フリック副文字を検証する。
- `C/A`のタップ非発火と、`BS`のタップ削除・下Esc・未割当方向の削除タップを検証する。
- あんの上下レイヤー移動、変換中でないEnterの上Paste・下C-j・中央tap・未割当左右・中心復帰・cancel、スワイプ中だけ変わるラベルを検証する。
- 日本語・テンキー左上の絵文字キー、?}、19、AZ、最下段レイヤーキーの4方向を検証する。テンキー`.`のtap・左右・未割当上下・中心復帰・cancelも検証する。
...
6. 絵文字レイヤー、Spaceカーソル操作、Fold7内外レイアウト、アクセシビリティを仕上げる。

...
日本語・テンキーレイヤーは左上の`☺`から絵文字レイヤーを開く。絵文字レイヤーはAndroidX Emoji Pickerの横scrollカテゴリicon headerを候補欄の50dp位置に置き、先頭Recentを含むカテゴリから一覧へ移動する。header下は8dpのgapと3行・8列の縦scroll grid、最下行は既存layer flickと削除を固定する。recentは端末内で新しい順・重複なし最大24件に保存し、private欄ではRecentへ表示・保存しない。日本語とテンキーの空白キーは`Space`とだけ表示するが、フリックで4方向カーソル移動へ入る。トラックパッドは動き始めの主方向から横軸または縦軸を選び、指を離すまで固定するため斜め移動はしない。

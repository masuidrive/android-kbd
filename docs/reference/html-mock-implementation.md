...
日本語・テンキーの左上`☺`は絵文字レイヤーを開き、進入ごとに先頭Recentと一覧先頭へ戻します。絵文字レイヤーは候補欄位置の横scrollカテゴリicon header（先頭Recent）と、かなレイヤー左端列と同じ幅の固定rail（上から`あ`、`?}`、`19`、`AZ`）、その右の最下段まで4行・7列のカテゴリ別縦scroll gridで構成します。wideのDual Flick時も一覧は7列を保ち、railだけかな左端列に合わせて12.5%になります。絵文字レイヤーに削除キーはありません。recentはlocalStorageで新しい順・重複なし最大100件に保存します。絵文字を選んだ後は、現在のカテゴリとスクロール位置を保ちます。日本語とテンキーの空白キーは `Space` と表示し、フリックすると上下左右のカーソル移動に入ります。テンキーの`.`はtapで`.`、左フリックで`,`、右フリックで`=`とし、上・下フリックは未割当です。QWERTYの`Space`は方向表示 `←↓↑→` を主ラベルの上に置き、トラックパッド開始時の中央ステータスにも同じ記号列を表示します。

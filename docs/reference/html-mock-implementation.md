...
開始点から高い11/標準15/低い19pxで軸を固定し、高い16/標準24/低い32pxで選択を開始します。選択後は高い9/標準13/低い17pxのヒステリシスで中心復帰を判定します。かな・テンキーとQWERTY・記号の感度は独立し、日本語は設定した選択距離以上で、横優勢なら左・右、縦優勢なら上・下を選びます。音声・絵文字は固定の旧標準（軸固定12px・選択18px・復帰10px）を使います。pointerupの最終座標も同じ判定へ通してから確定し、最後のpointermoveが間引かれても復帰またはフリックを反映します。

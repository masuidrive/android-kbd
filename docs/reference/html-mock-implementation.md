...
日本語レイヤーはCSS Gridで5列を固定します。左列は矢印を十字に配置したカーソルキー、#!、19、AZです。テンキーレイヤーでも同じカーソルキー、#!、あん、AZ、カーソルレイヤーではあん、#!、19、AZを並べます。左下は全レイヤー共通の切替キーで、左フリックは記号、上フリックは日本語、右フリックはテンキー、下フリックはQWERTYです。方向選択後は元の小さい補助文字を消し、移動先のレイヤー名だけを表示します。タップはQWERTY形式から日本語へ、テンキー形式からQWERTYへ移動します。テンキーの本体は `1 2 3 / 4 5 6 / 7 8 9 / - 0 .` とし、Enterキーは3行目右端を起点に4行目までの高さを持たせます。テンキーの−は中央−、左＋、上／、右＊、下，の5方向です。日本語とテンキーの空白キーは `Space` と表示し、フリックすると上下左右のカーソル移動に入ります。QWERTYの`Space`とカーソルレイヤーのSpaceは方向表示 `←↓↑→` を主ラベルの上に置き、トラックパッド開始時の中央ステータスにも同じ記号列を表示します。

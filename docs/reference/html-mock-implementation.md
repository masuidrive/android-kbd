...
QWERTYの1〜3行目は基本キーを `1w` とし、2行目だけ `C/A 0.5w + a〜l 9w + . 0.5w` にします。3行目は `?} + z〜, + BS` で合計10w。`?` は`AZ`と同じ重ね位置に淡い `}` を置く`?}`構成です。4行目の日本語切替キーも中央の `あ` と、同じ位置に重ねる淡い `ん` の`あん`構成にして、Space、Enterと並べます。`?}`、`あん`、`AZ`、`19` の補助文字は主ラベルの約75%に揃えます。記号レイヤーも同じ外形を保ち、`?` の位置は中央の `A` と右後ろの淡い `Z` を重ねたQWERTY復帰キーに変えます。変換中でないEnterは待機中に上側へ小さな `C-j`、中央に `Enter`、下側へ小さな `paste` を離して表示します。上スワイプ中は下側の `paste` が中央へ移動・拡大し、離した時に貼り付けを実行します。下スワイプ中は中央に `C-j` を表示し、離した時にCtrl+Jを実行します。

日本語・テンキーの左上`☺`は絵文字レイヤーを開き、進入ごとに先頭Recentと一覧先頭へ戻します。絵文字レイヤーは候補欄位置の横scrollカテゴリicon header（先頭Recent）と、かなレイヤー左端列と同じ幅の固定rail（上から`あ`、`?}`、`19`、`AZ`）、その右の最下段まで4行・7列のカテゴリ別縦scroll gridで構成します。wideのDual Flick時も一覧は7列を保ち、railだけかな左端列に合わせて12.5%になります。絵文字レイヤーに削除キーはありません。recentはlocalStorageで新しい順・重複なし最大100件に保存します。絵文字を選んだ後は、現在のカテゴリとスクロール位置を保ちます。日本語とテンキーの空白キーは `Space` と表示し、フリックすると上下左右のカーソル移動に入ります。テンキーの`.`はtapで`.`、左フリックで`,`、右フリックで`=`とし、未割当の上・下では`.`を入力します。QWERTYの`Space`は方向表示 `←↓↑→` を主ラベルの上に置き、トラックパッド開始時の中央ステータスにも同じ記号列を表示します。
...
英字は上スワイプで大文字、下フリックで副ラベルの文字です。横へ軸ロックした後は上下の入力へ変えません。`C/A` は記号の副ラベルと同じ高さになるよう少し内側へ固定表示し、下でCtrl、上でAltを選びます。選択中と選択後は中央にCまたはAを表示し、選択後のタップで解除します。未選択の`C/A`と`BS`はタップでも上スワイプでもポップアップを出しません。`BS`は下フリックだけで候補と削除を有効にします。レイヤー切替キーとEnterは方向判定中のラベルと背景色を更新し、指を離した時にレイヤー移動、上Paste、または下Ctrl+Jを確定します。

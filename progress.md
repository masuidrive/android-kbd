...
[2026/09/11 13:29] demo実装f26e370を独立browser/static確認。412pxは単一かな・横overflowなし、1000px内画面は左右2組のかなキー、Dual/layerはreload後も保持、console error/warning 0。pointer別stateとrelease/cancel処理をsource確認し、synthetic 2-pointer probeは逆順releaseで「かあ」、片方cancel後に他方releaseで「か」を保持した。実multi-touch端末試験ではなくbrowser合成eventとして区別する。

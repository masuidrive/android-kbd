...
[2026/09/14 20:17] v0.15.13 APKをGitHub Releaseへ直接公開し、再取得した38,616,144 bytesとSHA-256 `69ac5552dcc7b865f1533dce74d77de7c99225e57681f87dc354886516eb0f4e`がlocal artifactと完全一致した。製品ページはmasuidrive.jp `8aa4d19`へpushし、Pages run `34837267529`成功、公開4ファイルのbyte一致、412px・840pxの横overflowなし、音声layerの候補確定後の連続認識を実ブラウザで確認した。
[2026/09/14 20:20] ticket 260914-075602をクローズし、無音error 6・7から音声認識を自動再開するv0.15.13をmain `6ffdfd6`へ統合してoriginへpushした。
[2026/09/14 22:39] 音声入力中にマイク入力レベルへ追従する小さな波形を`認識中`表示の横へ出す作業をticket 260914-133849で開始した。既存`onRmsChanged`が値を捨てていることと、波形を高さ変更なしで収められる非操作slotを確認した。
[2026/09/14 23:18] 音声入力レイヤーの`認識中`横へ4本のリアルタイム波形を実装し、負値を含む入力レベルの単調表示、無音再開、候補待ち・取消・切替時の消去を回帰testで固定した。fast-checks、unit/lint/APK、API 36.1 AVDの接続testは3/3成功し、native音声layerの高さ不変も確認した。AVDに日本語端末内音声modelがないため、実マイクへの追従だけを実機human reviewへ残した。
[2026/09/15 00:02] ユーザが実マイクE2Eの未確認範囲を承知した上で、リアルタイム波形を絵文字レイヤー左列変更と合わせてpush・公開するよう明示承認した。波形ticketをcloseしてmainへ統合し、次の絵文字UI ticketへ進む。
...
[2026/09/15 16:55] release SHA `a155826`でfast-checks、全unit・lint・APK build、API 36 arm64 emulatorの接続testが3/3 PASSした。v0.15.14 APKは38,634,832 bytes、SHA-256 `852d9c6813fd6887eedceb456ecc7ea8104caa1e8881924bf3fc3b0a7d697f88`、package `com.masuidrive.gestureime`、versionCode 30、arm64-v8a、RECORD_AUDIOあり、INTERNETなし、zipalign成功を確認した。
[2026/09/15 17:01] v0.15.14をGitHub ReleaseへAPK単体で公開し、再取得した38,634,832 bytes、SHA-256 `852d9c6813fd6887eedceb456ecc7ea8104caa1e8881924bf3fc3b0a7d697f88`がlocal APKと完全一致した。masuidrive.jp `a23aa93`のPages run `34944490815`は成功し、公開4ファイルのbyte一致、412px・840pxの横overflowなし、readonly入力欄へのmockキー入力、wide時Dual Flickを確認した。
[2026/09/15 19:09] 絵文字レイヤーを、開くたびRecent、左列は日本語・記号・テンキー・QWERTY、右側はBackspaceなしの4行7列scroll一覧へ整えるticket 260915-100701を開始した。左列幅は表示中の日本語配列の左端列と同じ境界で検証する。
[2026/09/15 19:54] 絵文字Recent再進入をadapter position 0の実holderクリックまで保留し、input view再利用時のvisibility transitionも初期化した。Mockの選択後カテゴリ・scroll保持、wide Dualの7列、tap-only rail accessibility、現行実装ガイドを同期し、IME focused unitとAPI 36.1 AVDのproduction MotionEvent 6件を通過した。
...
[2026/09/24 23:02] Enter・記号Tabチケットをレビュー承認に基づきcloseしてmainへ統合した。v0.15.21の公開ticketを開始し、APKと製品サイトの公開先を確認した。
[2026/09/24 23:10] v0.15.21 APKをビルドし、接続24件・単体全285件・lintを確認した。単体1件は初回失敗後の再実行で通過し、独立レビューの重大指摘はなかった。
...
[2026/09/25 18:49] 未割当方向へのフリックを通常タップとして扱い、移動中の表示変化と追加振動を出さない調整を開始した。nativeと公開モックの現在の判定経路を調査する。
[2026/09/25 19:03] 未割当フリックの中央タップ確定と追加振動抑止、テンキー記号キーの方向ラベルをnativeとブラウザモックへ実装した。focused unit・接続テストとモック操作は通過し、独立レビューと全体検証に進む。
[2026/09/25 19:16] 独立レビューで未割当時に長押しが残る・記号補助ラベルが重複する・モックの一部キーが旧動作になる3件を検出した。nativeの2件を修正してfocused unitと接続試験を通し、モックの全経路を修正中。
[2026/09/25 19:43] 未割当フリックの5件のレビュー指摘を修正し、3085eeeでfast-checks・Android単体/lint/APK・接続25件の全区分が通過した。旧版動作が残ったマニュアルと技術資料を修正し、公開物の検証へ進む。
[2026/09/25 19:54] v0.15.22 APKをGitHub Releaseへ直接公開し、再ダウンロードのbyte一致を確認した。製品サイトもPagesの正確なcommitと公開4ファイル一致、412/840pxの実ブラウザ操作を確認したが、追加AC裏取りでQWERTY英字の未割当左右保持がアクセント長押しへ進むCriticalを検出したため、修正版の公開へ切り替える。
[2026/09/25 20:05] QWERTY英字の未割当左右保持でアクセント候補が出る反例をAPI36エミュレーターでも確認した。8496338で長押しタイマー取消経路を修正し、focused unitと412/840dpの接続10件が通過した。v0.15.23として独立レビューと最終検証へ進む。
[2026/09/25 20:14] v0.15.23の最終SHAでfast-checks・Android単体/lint/APK・接続26件が全通過した。直接APKをGitHub Releaseへ公開し、再ダウンロードした38,651,216バイトのbyte一致を確認した。製品サイトもPagesの正確なcommitと公開4ファイル一致、412/840pxの操作・横はみ出しなしを確認した。
[2026/09/25 20:20] 公開v0.15.23 APKをAPI36エミュレーター412/840dpで再確認し、英字未割当左保持は通常文字一回・追加振動なし、中央長押しのaccentは維持された。テンキー記号ラベルと未割当方向も実画面で確認し、AC裏取りは全件VERIFIEDでhuman review待ちへ進めた。
[2026/09/26 14:28] テンキー方式とQWERTYの個別フリック感度設定に着手した。現行は両者共通の選択18dp・中心復帰10dpで、設定値の保存・IME再表示時の反映経路を調査し、新チケットを作成した。
[2026/09/26 14:35] 新チケットへの分離を見直し、未割当フリックと同じ操作経路を扱う現行チケットへ感度設定のACを追加した。かな・数字とQWERTY・記号を独立した3段階設定としてnativeとモックへ実装中。
[2026/09/26 14:40] 二組の感度選択、IME再表示時の読込、モックの感度切替と判定距離の変更を実装した。保存と実タッチ経路のfocused検証、公開用v0.15.24の準備を進めている。
[2026/09/26 14:47] Android focused unit 82件と、API36.1エミュレーターの接続済みKeyboardView実MotionEvent試験が通過した。412/840dpの経路を確認し、独立レビューはCritical/Majorなし。二組の設定を実画面でも撮影し、最終全スイートへ進む。
[2026/09/26 14:51] 独立レビューの追加確認でモックの長押し取消が12px固定、資料に18px固定記述が残る2件のMajorを検出した。感度別閾値へ修正し、低感度の13px保持ではaccent表示・28px移動では抑止されることを実ブラウザで確認した。
[2026/09/26 14:53] 斜め移動では距離だけの判定が早すぎると再レビューで判明した。モックの長押し取消をnativeの実際の方向選択状態へ合わせ、低感度の斜め15×22pxではアクセント維持、15×27pxでは取消を確認した。
...
[2026/09/26 23:23] 全スイート初回は旧18dpを前提にした既存テストが6件失敗し、並列Gradleの単体コンパイルも一時的に失敗した。仕様書の旧値とテスト期待値を修正し、単体KeyboardViewと接続KeyboardView11件がPASS。各感度の境界前後、音声・絵文字の固定距離、中心復帰を追加検証した。
[2026/09/26 23:25] 最終コードSHA 83b8bccでfast-checks・Android単体/lint/APK・接続試験の全3区分がPASSした。独立再レビューで見つかった資料2ファイルの旧距離も新値へ修正し、v0.15.25公開準備を進める。
[2026/09/26 23:34] v0.15.25 APKをGitHub Releaseへ直接公開し、再ダウンロードした38,657,130バイトのbyte一致を確認した。製品サイトはPagesがeb545dbをbuiltと報告し、公開4ファイルをbyte照合した。公開412/840pxで両感度差のフリックを実操作し、横はみ出し・欠損画像・JSエラーがないことを確認した。
[2026/09/27 00:29] 「ゆるく」は確定までの移動を長くする意味だとユーザが訂正した。前回の短距離化は逆方向だったため、既存3段階を高い16・標準24・低い32dpへずらす案でチケットACと判断を更新し、nativeとmockの確定距離を調整する。
[2026/09/27 00:40] nativeとmockの判定を16/24/32dpへ変更し、接続済みKeyboardViewのfocused試験が通過した。全スイート初回は旧距離を前提にした別の接続試験2件が失敗したため、境界を越える距離へ試験を更新し、対象接続11件がPASSした。全スイートを最終SHAで再実行中。
[2026/09/27 00:49] 指離し最終座標の反映で中央長押しアクセントに回帰が出たため、アクセント選択更新をMOVE時に限定した。接続済みKeyboardViewの11件はPASS。全体試験初回の絵文字Robolectric試験1件は単独再実行でPASSし、同じコードSHAの全体再実行はfast-checks・Android単体/lint/APK・接続試験の3区分がPASSした。v0.15.26の公開成果物を検証する。
[2026/09/27 00:58] v0.15.26 APKをGitHub Releaseに直接公開し、再取得した38,854,722バイトがローカル成果物とbyte一致した。製品サイトはPagesがe1e6496をbuiltとして公開4ファイルがbyte一致。公開実ブラウザの412/840pxで高い・標準・低いの確定差、横overflowなし・欠損画像なし・JSエラーなしを確認した。物理Foldでの実指レビューを待つ。

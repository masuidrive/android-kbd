...
17. 日本語・テンキー左上の`☺`は絵文字レイヤーを開く。上3行はSharedPreferencesの新しい順・重複なし最大8件とAPK内catalogを空行なしで続ける縦scroll viewportにする。recentが空ならcatalog先頭24件を表示し、最下行は既存layer flickと削除を固定する。絵文字はStringのまま直接確定しrecent先頭へ保存する。旧`CURSOR`保存値はKANAへ安全にfallbackする。（2026-09-12 / 260912-020803-replace-cursor-layer-with-emoji, 260912-055128-make-emoji-grid-scrollable）

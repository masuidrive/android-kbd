...
18. IME入力Viewは候補欄固定50dp、intrinsic `WRAP_CONTENT`の`KeyboardView` 4行、中央シェブロンを表示する28dp hide barの3段を縦LinearLayoutへ積む。`KeyboardView`は入力先切替中に親から一時的な過大`EXACTLY`高を受けても、選択presetの4行高を上限として採用し、`onStartInput`と`onStartInputView`で保存値を再適用する。bottom insetはhide barが所有し、`KeyboardView`のpaddingへ加えないため、キー位置は動かない。`VOICE` modeと候補内容も候補欄・4行キーの高さを変えない。（2026-09-11 / 260911-063912-fix-intermittent-keyboard-vertical-offset; 2026-09-12 / 260911-160113, 260912-002431-stabilize-ime-height-after-app-switch, 260912-014943-fixed-keyboard-height, 260912-071844-add-ime-hide-bar）

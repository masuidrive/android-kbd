...
10. 音声入力は左下レイヤーキーを単一pointerで左へフリックし、方向が確定した時点で開始する。中央の1秒hold開始は廃止する。認識ready後に指を離すと最終結果を現在Editorへ自動確定し、候補欄からのStart/Stop/Confirm操作やpreview確定は提供しない。gesture generationとeditor sessionを検証し、中央tap/hold、上下右、複数pointer、入力欄切替、IME非表示、取消、通常キー入力後の古い結果を安全に扱う。（2026-09-11 / 260911-014049, 260911-055701-assign-layer-left-swipe-to-voice）

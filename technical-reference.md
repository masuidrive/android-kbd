...
17. 日本語・テンキー左上の`☺`は絵文字レイヤーを開く。最上行はSharedPreferencesの新しい順・重複なし最大8件を表示し、不足枠は空のままにする。残り2行はAPK内16件ずつのページ、最下行は既存layer flick、前頁、頁表示、次頁、削除で、全行を8 width unitに揃える。絵文字はStringのまま直接確定しrecent先頭へ保存する。旧`CURSOR`保存値はKANAへ安全にfallbackする。（2026-09-12 / 260912-020803-replace-cursor-layer-with-emoji）

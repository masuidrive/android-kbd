...
10. 音声入力は左下レイヤーキーを左へフリックし、方向が確定した時点で専用`VOICE` modeへ移って開始する。進入前の文字layerを保持し、候補tapまたは取消で戻る。VOICEレイヤーの左下キーは中央tapで取消、上で日本語、右でQWERTY、下でテンキーへ切り替える。（2026-09-11 / 260911-014049, 260911-055701-assign-layer-left-swipe-to-voice; 2026-09-12 / 260911-160113）
...
15. 日本語変換候補と英字補完候補は共通faceを使い、高さ34dp、最小幅82dp、左右padding 14dp、gap 5dp、radius 7dp、1dp下影、HTML 15px相当の固定15dp文字で描画する。候補欄全高50dpではfaceをtop 14dp/bottom 2dpに置き、最上段key faceまでをキー行と同じ10dpにする。先頭face左はphone幅6dp、600dp以上のwide幅13dpで最左key faceと揃え、view幅変更時にも再適用する。Light/Darkの通常/選択色、横scroll reset、音声の許可・非対応controlの操作可能性は維持する。（2026-09-11 / 260911-062719-align-candidate-ui-with-html; 2026-09-12 / 260912-002431-align-native-settings-and-voice-ui, 260912-112512-align-candidate-strip-spacing）
...
17. 日本語・テンキー左上の`☺`は絵文字レイヤーを開く。上3行はSharedPreferencesの新しい順・重複なし最大8件とAPK内catalogを空行なしで続ける縦scroll viewportにする。recentが空ならcatalog先頭24件を表示し、最下行は既存layer flickと削除を固定する。絵文字はStringのまま直接確定しrecent先頭へ保存する。旧`CURSOR`保存値はKANAへ安全にfallbackする。（2026-09-12 / 260912-020803-replace-cursor-layer-with-emoji）
18. IME入力Viewは候補欄を固定50dp、`KeyboardView`をintrinsic `WRAP_CONTENT`として縦LinearLayoutへ積む。`KeyboardView`は入力先切替中に親から一時的な過大`EXACTLY`高を受けても、選択presetの4行高を上限として採用し、`onStartInput`と`onStartInputView`で保存値を再適用する。遅延したbottom insetはキー位置を動かさない末尾safe areaであり、`VOICE` modeと候補内容もキー領域高を変えない。（2026-09-11 / 260911-063912-fix-intermittent-keyboard-vertical-offset; 2026-09-12 / 260911-160113, 260912-002431-stabilize-ime-height-after-app-switch, 260912-014943-fixed-keyboard-height）
...
21. 端末内音声認識はpartial resultsを要求し、最新の途中結果を通常候補欄の共通faceへ表示する。音声の途中結果と最終候補だけは候補欄の可視幅以下で最大2行に折り返し、超過分を末尾省略する。最終結果は空文字と重複を除いた可変件数を同じ候補UIへ順序どおり表示し、描画tokenとeditor sessionが一致する候補tapだけを1回確定する。取消、error、IME終了、入力欄切替では未確定結果を破棄する。日本語変換、英字補完、スラッシュ候補は従来の1行表示と横スクロールを保つ。（2026-09-11 / 260911-060238-align-voice-status-ui-and-show-partials; 2026-09-12 / 260911-160113, 260911-234748）

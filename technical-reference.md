...
17. 日本語・テンキー左上の`☺`は絵文字4行面を開く。最上行はSharedPreferencesの新しい順・重複なし最大8件を表示し、不足枠は空のままにする。残り2行はAPK内16件ずつのページ、最下行は既存layer flick、前頁、頁表示、次頁、削除で、全行を8 width unitに揃える。絵文字はStringのまま直接確定しrecent先頭へ保存する。旧`CURSOR`保存値はKANAへ安全にfallbackする。（2026-09-12 / 260912-020803-replace-cursor-layer-with-emoji）
17. IME入力Viewは候補欄を固定50dp、`KeyboardView`をintrinsic `WRAP_CONTENT`として縦LinearLayoutへ積む。`KeyboardView`は入力先切替中に親から一時的な過大`EXACTLY`高を受けても、選択presetの4行高を上限として採用し、`onStartInput`と`onStartInputView`で保存値を再適用する。遅延したbottom insetはキー位置を動かさない末尾safe areaであり、`VOICE` modeと候補内容もキー領域高を変えない。（2026-09-11 / 260911-063912-fix-intermittent-keyboard-vertical-offset; 2026-09-12 / 260911-160113, 260912-002431-stabilize-ime-height-after-app-switch, 260912-014943-fixed-keyboard-height）

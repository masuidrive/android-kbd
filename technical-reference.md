...
21. 端末内音声認識はpartial resultsを要求し、最新の途中結果と状態を専用音声panelへ表示する。`onEndOfSpeech`後500ms以内に最終結果がない場合、または`NO_MATCH`・`SPEECH_TIMEOUT`・`CLIENT` errorで終了した場合は、最後の有効な途中結果を選択可能な最終候補へ昇格する。最終候補は空文字と重複を除き、1行1候補の固定高faceとして縦scroll一覧へ順序どおり表示する。複数候補では各候補の文字列を文字単位で比較し、全候補に共通しない文字だけを太字とaccent色で示す。同一候補だけ、または1候補だけなら強調しない。描画token、editor session、連続session generationが一致する候補tapだけを1回確定して次回認識へ戻す。取消、その他のerror、IME終了、入力欄切替では未確定結果を破棄し、自動再試行しない。日本語変換、英字補完、スラッシュ候補は従来の1行表示と横スクロールを保つ。（2026-09-11 / 260911-060238-align-voice-status-ui-and-show-partials; 2026-09-12 / 260911-160113, 260911-234748, 260912-122938-keep-voice-listening-until-cancel; 2026-09-14 / 260914-013111-highlight-voice-candidate-differences）

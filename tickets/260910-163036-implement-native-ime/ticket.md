...
- [ ] AC 1: 利用者が IME を有効化して入力欄を選ぶと、native の Custom View に「日本語」「テンキー」「カーソル」「QWERTY」「記号」の5レイヤーが仕様のキー配列で表示され、レイヤー切替キーのタップまたは4方向フリックで移動できる。
- [ ] AC 2: 利用者が日本語キーを中央・左・上・右・下へ操作すると12キーの対応するかなが連打なしで composing text になり、18dp未満の揺れでは別方向が誤確定しない。
- [ ] AC 3: 利用者が QWERTY の各英字キーをタップ、上スワイプ、下フリックすると、小文字、1文字だけの大文字、仕様の数字またはASCII記号がそれぞれ入力され、C/A と BS は仕様で許可された方向だけ発火する。
- [ ] AC 4: 利用者が Space を方向へ動かすと、開始時に決まった横軸または縦軸を指を離すまで維持してカーソルが移動し、絵文字や結合文字を途中で分割しない。
- [ ] AC 5: 利用者がかなを入力すると Mozc 接続から候補が表示され、Space で候補を巡回し、候補タップまたは Enter で対象アプリへ確定できる。Mozc を利用できないビルドでも同じ変換境界のテスト実装で一連の状態遷移を検証できる。
- [ ] AC 6: 利用者の文字入力、削除、Enter、Paste、Ctrl/Alt が InputConnection を通じて対象アプリへ届き、パスワード欄では候補学習と Paste が無効になる。

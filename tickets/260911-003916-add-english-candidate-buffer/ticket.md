...
### Design Decisions
- 英数字候補は、コード・ターミナル用途を考慮して設定toggleで明示的に有効化する案とする。既定値は未決であり、ticket reviewで決定する。
- 候補辞書または候補生成方式、対象言語・文字種、単語境界の定義は未決であり、着手時に端末内処理だけで設計を決定する。
...
- この起票時点での実装、設定追加、既存ticketのclose。

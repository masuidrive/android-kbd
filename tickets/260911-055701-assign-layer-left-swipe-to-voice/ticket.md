...
- [x] AC 1: レイヤーキーを左へスワイプして方向が確定すると端末内音声入力を開始し、そのまま指を離すと認識を終了して確定する。
- [x] AC 2: 左スワイプは記号レイヤーへ移動せず、他の上下右とタップの割当は維持される。
- [x] AC 3: パスワード欄と端末内認識非対応時は既存の安全な非表示・非対応動作を維持する。
...
- レイヤーキー中央の長押し音声は廃止し、左方向が選択された時点でpush-to-talkを開始する。

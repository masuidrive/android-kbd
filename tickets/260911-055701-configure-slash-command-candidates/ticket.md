...
- [x] AC 1: 設定画面でスラッシュコマンド候補を最大6件入力・保存できる。
- [x] AC 2: 初期値として`/compact`、`/clear`、`/quit`が設定され、残りの欄は空である。
- [x] AC 3: 通常入力欄で`/`を入力すると保存済み候補が表示され、候補をタップすると入力中の`/`を選んだコマンドへ置き換えて確定する。
- [x] AC 4: 空欄と重複候補は表示せず、パスワード欄では候補を表示しない。

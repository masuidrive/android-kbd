...
- [x] AC 5: 設定で「大」を明示選択した利用者が別アプリへ切り替えて入力欄をfocusしたときも、4行のキー領域が縮まず「大」の高さを維持する。「小」「標準」を明示選択した場合も、それぞれの高さを維持する。
...
- v0.15.8で未選択時の既定を「大」へ変えた判断は、設定済みの「大」がアプリ切替後に縮む添付画像の再現になっていなかった。既定値と切替時の高さ保持を分けて扱い、明示保存した「大」のまま同じ症状を再現してから原因を直す。

...
## 260911-102026-add-enter-up-flick-ctrl-j

### Why
<!-- ユーザ価値・解きたい問題を 1〜3 行で書く。
     Product Brief の Problem / Solution のどの部分を担うか明記する。 -->
ターミナル等で改行制御文字を素早く送れるよう、既存のEnter/PasteキーへCtrl+J操作を追加する。
...
このticketが終わると、Gesture IME利用者がEnterキーの上フリックでCtrl+Jを送信できる。

- [x] AC 1: 変換中でないEnterを上フリックすると、対象アプリへCtrl+Jが送信される。
- [x] AC 2: Enter待機中に上方向の補助ラベルは表示されず、上フリック選択中だけ`C-j`が拡大表示される。
- [x] AC 3: Enterのtap改行と下フリックPaste、日本語変換中のtap確定・上無変換・左カタカナは維持される。
- [x] AC 4: 公開demoでも同じ表示と操作を試せる。
...
Ctrlを次のキーへ保持せず同じgesture内でJへ適用するため、AI-1〜AI-4と矛盾しない。

### Design Decisions
<!-- 既知の設計判断と理由を箇条書きで明示。
     例: - データ保存形式: data URI (Files API は将来 ticket、本 ticket では不要)
     例: - 423 reject ではなく 422: validation error として扱う -->
- Ctrl+Jは`KeyEvent.KEYCODE_J`のCtrl修飾down/upとして送る。
- 変換中は既存の上フリック無変換を優先する。
- `C-j`は選択中だけEnter中央へ拡大表示する。

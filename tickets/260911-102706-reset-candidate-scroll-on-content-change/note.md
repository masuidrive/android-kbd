# Work Notes: 260911-102706-reset-candidate-scroll-on-content-change
...
- [-] PDH-implement: 外部 provider 経由 path は実 API 200 確認済み (deferred の場合は明示記録) - skip: 外部providerを使わない描画状態実装
...
- [x] Android view testで候補内容変更時scrollX=0、同一候補の選択変更時scrollX維持を確認。demoも実ブラウザで37から0への復帰を確認した
...
|---|---|---|---|---|---|
|   |      |     |      |      |      |
| 1 | 最終レビュー | - | Critical/Majorなし | 採用 | nativeとdemoの同一条件を確認 |
...
ユーザの直接指示を承認済み契約として確認。候補内容変化と選択変化を分けるACは観察可能。
- `8bbacf7`: CandidateStripViewで候補配列の内容が変わった場合だけ即時scrollTo(0, 0)。

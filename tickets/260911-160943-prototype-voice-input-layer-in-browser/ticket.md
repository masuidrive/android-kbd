...
このticketが終わると、閲覧者がブラウザモックで専用音声入力面の開始から候補選択、送信、取消まで試せる。

- [ ] AC 1: 文字レイヤーのレイヤーキーを左へフリックすると音声入力面へ移り、その時点で模擬認識を開始する。
- [ ] AC 2: 音声入力面は認識状態と更新される途中結果を表示し、最終結果ができるまで入力欄へ文字を追加しない。
- [ ] AC 3: 最終結果は候補欄へ複数表示し、候補をタップすると送信対象の選択表示が切り替わる。
- [ ] AC 4: 「送信」で選択中の候補を入力欄へ1回だけ追加して直前の文字レイヤーへ戻る。
- [ ] AC 5: 「キャンセル」で文字を追加せず直前の文字レイヤーへ戻る。
- [ ] AC 6: トップ埋め込みと独立demoのMobile/Tablet、Light/Dark、Dual Flickを壊さず、音声面でも横overflowがない。
...
- 既存の候補face、キー形状、Light/Dark tokenを再利用する。

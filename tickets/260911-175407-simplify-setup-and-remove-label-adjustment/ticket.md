...
- [x] AC 1: セットアップ画面とアプリ内の導線からQWERTYラベル調整機能がなくなり、以前に保存した調整値もキー表示へ影響しない。
- [x] AC 2: セットアップ画面に、インストールされているアプリのバージョンが表示される。
- [x] AC 3: セットアップ画面で初期設定、入力設定、スラッシュ候補、アプリ情報のまとまりが見分けられ、各操作を48dp以上のタップ領域から実行できる。
- [x] AC 4: Light/Darkの両テーマとスマホ・タブレット相当の幅で、文字や操作が欠けず縦スクロールで全項目へ到達できる。
- [x] AC 5: 既存のIME有効化、IME選択、入力テスト、各トグル、マイク権限、スラッシュ候補保存、ライセンス表示が引き続き動作する。

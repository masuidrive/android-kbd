...
- [ ] AC 1: トップ埋め込みデモのキーボード上部に、初期文「ここは入力できるよ」と以後の入力結果が表示される。
- [ ] AC 2: 表示された結果はキー入力、削除、候補確定、音声候補確定に追従する。
- [ ] AC 3: 入力欄は枠線なしで表示され、Light/DarkとMobile/Tabletで横overflowを生じない。
- [ ] AC 4: 単独の操作モックでは従来の編集可能な入力欄を維持する。

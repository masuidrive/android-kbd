...
## 260912-001139-show-hero-demo-input-result

### Why
製品トップの操作デモは入力欄全体を非表示にしているため、キーを操作しても入力結果を確認できない。
入力欄を復活させ、枠線なしのままキーボード操作の結果をその場で読めるようにする。
...
このticketが終わると、製品トップのデモを操作する利用者が入力結果をその場で確認できる。

- [x] AC 1: トップ埋め込みデモのキーボード上部に、初期文「ここは入力できるよ」と以後の入力結果が表示される。
- [x] AC 2: 表示された結果はキー入力、削除、候補確定、音声候補確定に追従する。
- [x] AC 3: 入力欄は枠線なしで表示され、Light/DarkとMobile/Tabletで横overflowを生じない。
- [x] AC 4: 単独の操作モックでは従来の編集可能な入力欄を維持する。

### Architectural Invariants check
静的mock内の表示変更だけであり、Android IMEと端末内処理の設計に影響しない。
...
- 既存textareaを入力状態の正本として維持し、hero埋め込みでも入力欄を表示する。
- heroでは既存の初期文「ここは入力できるよ」を表示し、textareaの枠線は付けない。
...
ユーザ指示: 「デモの入力結果でないのは治った？次はそれやってね。」
追加確認: 「入力枠はいらない」は入力欄全体の削除ではなく、枠線だけ不要という意味。入力欄は表示する。

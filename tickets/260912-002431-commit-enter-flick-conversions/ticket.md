...
- [ ] AC 1: 日本語変換中にEnterを上へフリックすると、元の読みをひらがなのまま即時確定し、変換状態を終了する。
- [ ] AC 2: 日本語変換中にEnterを左へフリックすると、元の読みを全角カタカナへ変換して即時確定し、変換状態を終了する。
- [ ] AC 3: どちらのフリックも改行や追加文字を挿入せず、次の入力は新しい読みとして始まる。
- [ ] AC 4: 変換中Enterのタップによる現在候補の確定と、変換していないEnterの既存操作は維持される。
- [ ] AC 5: ブラウザmockも同じ1フリック即時確定になり、確定後に候補欄が空になる。

...
## 260912-002431-commit-enter-flick-conversions

### Why
日本語変換中のEnter上フリックと左フリックは、現在は無変換またはカタカナのpreviewへ切り替えた後に再度確定操作が必要になる。
選択した表記が決まっているため、1回のフリックで入力先へ確定してタップ回数を減らす。
...
このticketが終わると、日本語入力中の利用者がEnterの1回のフリックで無変換またはカタカナを確定できる。

- [x] AC 1: 日本語変換中にEnterを上へフリックすると、元の読みをひらがなのまま即時確定し、変換状態を終了する。
- [x] AC 2: 日本語変換中にEnterを左へフリックすると、元の読みを全角カタカナへ変換して即時確定し、変換状態を終了する。
- [x] AC 3: どちらのフリックも改行や追加文字を挿入せず、次の入力は新しい読みとして始まる。
- [x] AC 4: 変換中Enterのタップによる現在候補の確定と、変換していないEnterの既存操作は維持される。
- [x] AC 5: ブラウザmockも同じ1フリック即時確定になり、確定後に候補欄が空になる。

### Architectural Invariants check
既存のcompositionをInputConnectionへ1回だけcommitし、端末内Mozcと固定IMEレイアウトの方針を維持する。
...
- previewを経由せず、上・左フリックの選択結果をexactly onceでcommitする。
- Enterキーの補助ラベルは追加せず、既存の変換中表示「確定」を維持する。

### Out-of-scope
<!-- やらないこと (scope creep 防止)。
     「ついでにやりそう」「次の ticket でやる」を明記する。 -->
- Mozc候補生成・学習・ユーザー辞書の変更。
- 通常時EnterのCtrl+J/Paste割当変更。

▼ 以下は該当する情報がある場合のみ ▼

### Implementation Notes
ユーザ指示: 「変換の時にenterのフリックで未確定やカタカナにするやつのはそのまま確定していいよ。」
「未確定」は既存の上フリック無変換（元readingのひらがな）を指すものとして実装する。

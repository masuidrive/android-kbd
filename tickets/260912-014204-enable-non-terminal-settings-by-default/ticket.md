...
- [ ] AC 1: 保存値がない場合、Dual Flick、英数字候補、Android個人辞書がONになる。
- [ ] AC 2: 保存値がない場合、ターミナル向けカーソル操作はOFFのままになる。
- [ ] AC 3: 利用者が各switchを変更した保存値は再起動後も優先され、default変更で上書きされない。
- [ ] AC 4: 設定画面のswitch表示とマニュアルの初期値説明が新しいdefaultに一致する。

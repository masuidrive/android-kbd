...
- [x] AC 1: 設定画面でキーボードの高さを「小・標準・大」から選択でき、選択値が再起動後も維持される。
- [x] AC 2: 後発のユーザー指定による初期値「大」では、スマホ幅・タブレット幅、Dual FlickのON/OFF、全レイヤーでキー4行の外形高が同じになる。
- [x] AC 3: IME初回表示、アプリ切替、Foldの開閉、画面回転後に、設定した高さからキー位置とタップ領域がずれない。
- [x] AC 4: 候補欄の高さ、キー間隔、ポップアップ、フリック判定、音声レイヤーの4行構成を維持する。
- [x] AC 5: 設定値が壊れている場合は後発のユーザー指定による「大」へ戻り、IMEが表示不能にならない。
...
- 初期値は当初「標準」だったが、2026-09-13の実機画像確認と「高い方が正解」という追加指定により「大」へ変更した。明示保存済みの高さは変更しない。
...
`260912-021714-fix-setup-safe-area-and-app-bar`。同じSetup画面へ高さ選択を追加する前に、トップアプリバーとsafe areaを確定する。

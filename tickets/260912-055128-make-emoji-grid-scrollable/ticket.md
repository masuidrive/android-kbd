...
- [ ] AC 1: 絵文字レイヤーを開くと、最下段のレイヤー操作行を除く上3行が絵文字一覧として表示される。
- [ ] AC 2: 上3行を上下へドラッグすると、固定4行高を変えずにrecentと全絵文字を連続してスクロールできる。
- [ ] AC 3: recentが空のときは一覧の先頭24件が3行を埋め、recentがあるときは先頭に新しい順・重複なし最大8件を置いて残りを続ける。
- [ ] AC 4: 前頁・頁表示・次頁キーを表示せず、絵文字tap、削除、最下段レイヤーフリックは維持される。
- [ ] AC 5: Android nativeと公開操作mockで同じ3行viewport・連続scrollになり、Light/Darkとスマホ/タブレット幅で横overflowしない。

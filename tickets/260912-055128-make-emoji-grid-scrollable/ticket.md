---
priority: 2
base_branch: features/260912-020803-replace-cursor-layer-with-emoji
description: "Make the emoji grid vertically scrollable and fill three visible rows"
created_at: "2026-09-12T05:51:28Z"
started_at: 2026-09-12T05:52:48Z # Do not modify manually
closed_at: null   # Do not modify manually
canceled_at: null # Do not modify manually
---

## 260912-055128-make-emoji-grid-scrollable

### Why
現在の絵文字レイヤーは一覧を2行ずつページ分割したため、広い入力領域に対して一度に見える絵文字が少なく、前後キーと頁表示も選択を遅くしている。
絵文字を連続して探せるスクロール一覧へ変え、固定キーボード高の中で一覧領域を有効に使う。
<!-- ユーザ価値・解きたい問題を 1〜3 行で書く。
     Product Brief の Problem / Solution のどの部分を担うか明記する。 -->

### What / Acceptance Criteria
<!-- 完了を判定できる条件。プロダクトの観察可能な振る舞いだけを書く。
     読み手はこの ticket を承認する人であり、実装する agent ではない。

     箇条書きを書き始める前に、この節の冒頭へ 1 文を書く:
       「この ticket が終わると、〈誰〉が、いままでできなかった〈何〉をできるようになる」
     各 AC はその 1 文の分割として書く。非退行の AC だけは例外で、〈誰〉のみ必須。
     読めているかの判定は pdh-dev skill の「AC に書いてよいもの / 書いてはいけないもの」に従う。

     例: 「新しく登録したユーザが、一覧の画面に出る」
     例: 「画面幅 375px 以下でメニューがハンバーガーに切り替わる」

     プロセス要件 (レビュー済み、テストパス等) はここには書かない。
     ワークフロー (SKILL.md) と作業ノート (note) が保証する。

     runtime で UX/Security invariant を強制する ticket では、AC に「runtime enforce の
     保証メカニズム」を 1 行明記する (例: editor 警告だけでなく 422 reject されること)。 -->
このticketが終わると、利用者が絵文字レイヤーの上3行をスクロールし、ページ送りをせず連続した一覧から絵文字を選べる。

- [x] AC 1: 絵文字レイヤーを開くと、最下段のレイヤー操作行を除く上3行が絵文字一覧として表示される。
- [x] AC 2: 上3行を上下へドラッグすると、固定4行高を変えずにrecentと全絵文字を連続してスクロールできる。
- [x] AC 3: recentが空のときは一覧の先頭24件が3行を埋め、recentがあるときは先頭に新しい順・重複なし最大8件を置いて残りを続ける。
- [x] AC 4: 前頁・頁表示・次頁キーを表示せず、絵文字tap、削除、最下段レイヤーフリックは維持される。
- [x] AC 5: Android nativeと公開操作mockで同じ3行viewport・連続scrollになり、Light/Darkとスマホ/タブレット幅で横overflowしない。

### Architectural Invariants check
絵文字一覧とrecentは引き続きAPK内データと端末内保存だけを使い、入力内容や履歴を外部へ送らない。
<!-- product-brief.md の Architectural Invariants と矛盾しないことを 1 行宣言する。
     矛盾しない場合: 「Hub stateless / Process immutable と矛盾しない」等。
     新規 Invariant を要求する場合: 実装を止めて Product Brief 更新から始める。 -->

### Design Decisions
<!-- 既知の設計判断と理由を箇条書きで明示。
     例: - データ保存形式: data URI (Files API は将来 ticket、本 ticket では不要)
     例: - 423 reject ではなく 422: validation error として扱う -->
- スクロール方向は上下とし、上3行をviewport、最下段を固定操作行にする。
- recentとcatalogを一つの連続一覧として並べ、空recent用の空行は作らない。
- ドラッグ距離は行単位に制限せず追従させ、端では範囲内に収める。

### Out-of-scope
<!-- やらないこと (scope creep 防止)。
     「ついでにやりそう」「次の ticket でやる」を明記する。 -->
- 絵文字検索、カテゴリタブ、一覧内容の追加。
- キーボードの4行高、recent最大8件、Spaceカーソル操作の変更。

▼ 以下は該当する情報がある場合のみ ▼

### Implementation Notes
ユーザ指示: 「絵文字はスクロールにするし、なぜ2行？」。添付実機画像では一覧2行とページ操作行が表示されていた。
<!-- ユーザの明示指示、またはユーザが会話で言及した事項のみ書く (関数名 / module 名レベルまで)。
     設計判断は「Design Decisions」に書く。
     Coding Engineer は Implementation Notes が空でも実装できる責務を持つ。
     PM が自主的に実装詳細を書いてはならない (下流の自由度を奪う)。 -->

### Dependencies
`260912-020803-replace-cursor-layer-with-emoji`の実装をbaseとする。
<!-- この ticket に着手するために完了が必要な他の ticket。
     「参考情報」ではなく「ブロッカー」だけ書く。なければ省略。
     ブロッカー = これが未完了だと実装・テストが物理的にできない依存。
     例: 「DB migration の ticket が先に必要」「認証 API が存在しないと結合できない」
     参考情報 (設計の参考にした ticket 等) は書かない。
     coding agent は未完了の依存がある場合、着手せず報告する。 -->

---
Work notes: `note.md`

...
このticketが終わると、利用者が`masuidrive-kbd`として統一された画面で、キーボード下端の操作からIMEを閉じ、入力テスト画面の上部ナビゲーションから設定画面へ戻れる。
...
- [ ] AC 4: 閉じる操作行と入力テスト画面のナビゲーションはLight/Dark、412dp/840dp幅で欠けず、Androidのsystem status barやsystem navigation barへ重ならない。

...
- [x] AC 13: 音声入力レイヤーの最下段は、キャンセル、認識状態、「、。？！」、Space、Backspace、Enterの6列になる。Backspaceはtapで現在の入力欄から1文字削除し、フリック操作は割り当てない。録音中・認識処理中・候補表示中に削除しても音声session、認識中表示、未確定の音声候補を終了・再開始・消去せず、nativeと製品トップの操作mockで同じ配置と動作になり、412dp・840dpと小・標準・大の固定4行高を維持する。

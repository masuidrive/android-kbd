...
このticketが終わると、利用者が音声レイヤーで認識状態を確認しながら、キャンセルするまで候補選択と次の認識を繰り返せる。

- [x] AC 1: 連続音声入力session中（録音・認識処理・最終結果の候補選択待ち・次回認識開始中）は、最下段左のキャンセルキー右隣に「認識中」と分かる表示が出る。権限未許可・モデルなし・認識不能では既存の状態・エラー表示を出し、認識中とは表示しない。
- [x] AC 2: 最終結果として表示された候補のいずれかをタップすると現在の入力欄へ1回だけ確定し、旧候補を消して音声レイヤーのまま次の認識状態へ戻る。追加操作なしの次発話が新しい候補として表示される。
- [x] AC 3: 対応環境の通常フローでは候補選択と次の認識を2回以上繰り返せる。キャンセルをタップすると認識・候補を破棄して進入前の文字レイヤーへ戻り、その後の旧callbackを表示・確定しない。
- [x] AC 4: private欄では音声入力を開始しない。通常欄からprivate欄・別editorへの切替、IME非表示、別layerへの切替時は停止し、切替前の途中結果・最終結果・候補tapは入力も次の認識も起こさない。認識error時は既存表示を出し自動再試行しない。
- [x] AC 5: 録音から候補、再開始までを2周してもAndroid nativeと公開操作mockの全体4行高と最下段touch targetが変わらず、412dp・840dpでキャンセル、状態表示、既存の上下右flickが欠けない。
...
- 認識不能・権限未許可・モデルなしは現在のエラー表示を維持し、自動再試行しない。

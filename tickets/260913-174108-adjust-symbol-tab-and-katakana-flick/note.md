...
- [x] WhyはProduct BriefのProblem/Solutionへ接続し、AC 1〜5は端末またはmockで観察可能。
- [x] ユーザが具体的な配置・表示・フリック動作と公開までを明示したため、未承認の設計判断はない。
- [x] Design Decisions、Out-of-scope、Architectural Invariantsを確認し、依存ticketはない。
...
- [x] 現行QWERTYでは`m`下フリックが`:`で、Symbolsの直接入力`:`を削除しても文字は残る。
- [x] 既存unit testがQWERTY＋SymbolsのSpace〜`~`の印字可能ASCII 95文字を完全一致で検証している。
- [x] 現行Symbols 3行目は10列であり、`:`を`Tab`へ一対一で置換すればレイアウト寸法は変わらない。
- [x] 現行実装にはTab actionがないため、Android `KEYCODE_TAB`のdown/upを送る経路と回帰testを追加する。

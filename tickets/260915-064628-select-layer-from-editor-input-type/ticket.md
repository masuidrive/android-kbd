...
- [x] AC 1: 数字、符号付き数、小数、数字パスワード、電話番号、日時の入力欄では、最初からテンキーレイヤーが表示される。
- [x] AC 2: URL、メール、Webメール、文字パスワード、可視パスワード、Webパスワード、および数字・電話・日時以外でASCIIを強制する入力欄では、最初からQWERTYレイヤーが表示される。
- [x] AC 3: 通常の文章、名前、住所、メッセージ、未指定または未知の入力種別では、利用者が最後に明示選択したレイヤーが表示される。
- [x] AC 4: 入力種別による自動選択だけでは最後の明示選択を変更せず、入力中は従来どおり手動で別レイヤーへ切り替えられる。
- [x] AC 5: 最初の表示、入力欄・アプリの切替、入力ビューの再生成でも、スマホ幅とタブレット幅のキーボードの高さや安全領域が崩れない。

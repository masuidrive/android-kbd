# Work Notes: 260916-095653-animate-rms-pseudo-spectrum
...
- [x] 測る対象を洗い出し、書き手が測れるものは測って結果をここへ書いた: Android `RecognitionListener`から得られる表示用入力は単一の`onRmsChanged(Float)`で周波数帯情報はない。現行native/mockは4本を固定係数`[.55, 1, .72, .42]`で同時に拡縮する。専用status slot内の現行波形幅は10.5dpで、6本を1.2dp幅・1dp間隔にしても12.2dpに収まり、slotと4行高を変える必要はない。
...
2026-09-16: WhyはProduct Briefの音声レイヤーと端末内完結へ接続する。ACはRMS通知時の6本表示、同一段階での変化、終了時の消去、レイアウト・アクセシビリティ非退行、mock parityとして観察可能。ユーザの「疑似でやってみて」をこのticket contractへの実装承認として記録する。実周波数解析と公開は明示的にscope外とした。

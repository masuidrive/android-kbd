...
- [x] 測る対象を洗い出し、書き手が測れるものは測って結果をここへ書いた
  - 現行versionCode 33・versionName 0.15.17、直前公開版v0.15.17のため次版を34・0.15.18とする。
  - Release assetはAPK 1個とし、再取得byte一致、metadata、ABI、permissionを測る。
  - 公開サイト4主要fileのbyte一致、Pages成功、412px・840px overflowとBackspace mock操作を測る。
...
| # | 観点 | Sev | 要旨 | 判定 | 理由 |
|---|---|---|---|---|---|
| - | release整合 | - | Critical / Major / Minor 0件 | 採用 | 独立reviewerがcommit `ac5def6e`のversion、APK導線、manual、release notes、mock正本一致、scopeを確認した。公開状態は公開後に検証する。 |

## Technical reference 更新
<!-- この ticket の差分に因果がある追記・上書きの内容、または「該当なし」＋理由を 1 行以上必ず書く。
     他 ticket 由来の記述を消したくなったら、消さずにここへ削除候補として記録する。 -->
該当なし。振る舞いは直前ticketで正本へ反映済みで、本ticketは版番号、配布導線、利用者向け変更履歴だけを更新する。
...
2026-09-19: ユーザの「apk公開、ページ更新」をmain push済みBackspace Esc補助ラベル修正版のGitHub Release・製品ページ公開・release ticket close承認として扱う。

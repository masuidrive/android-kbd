...
## Status: PDH-ticket-human-review (Approved by explicit implementation and publication request)
...
- [x] 測る対象を洗い出し、書き手が測れるものは測って結果をここへ書いた
  ...
  - nativeの候補選択状態は`conversionPreview`、mockは`composition.converted`で識別できる。候補欄タップは即時確定するため変更対象外とする。

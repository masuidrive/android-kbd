...
- [ ] AC 1: 日本語の読みを入力して候補が表示され、まだ候補を選んでいない間はSpaceが「候補」と表示され、タップすると候補を選択する。
- [ ] AC 2: 候補を選択した後はEnterが「確定」と表示され、タップすると表示中の候補を入力欄へ確定する。
- [ ] AC 3: 候補選択前のEnterは従来どおり「無変換」で、上・左フリックのカタカナ変換も維持する。候補選択後もSpaceの「候補」で次候補へ進める。
- [ ] AC 4: native、操作mock、正本仕様、使い方ページが同じ状態遷移を示し、公開v0.15.19 APKと製品ページから利用できる。
...
- 「候補を選んだ」はSpaceによる変換候補previewが存在する状態とする。候補欄のタップは従来どおり即時確定する。

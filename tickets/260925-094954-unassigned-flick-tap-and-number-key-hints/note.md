...
- [x] ユーザ訂正: フリック確定はもっと長い移動を必要とし、高いを旧標準程度にする。nativeとmockを揃えて公開する
...
- 2026-09-27: 前行の解釈はユーザにより明示的に否定された。「ゆるく」は確定までの移動を長くする意味。提案された「標準を高い程度へずらす」案を採用し、既存3段階を高い16/標準24/低い32dpへ移す。4段階目追加より設定操作・保存形式・mockの変更が少なく、旧標準に慣れた人は高いを選べる。方向popupの選択と指離し時の確定は現行同一の選択状態を使うため、popupだけ旧距離で先行する仕様にはしない。

...
- [x] AC 8: 「かな・テンキー」「QWERTY・記号」の各感度で、v0.15.25より長く指を動かして初めて割当済みフリックが確定する。「高い」は旧「標準」と同程度の距離となり、途中で離したときはタップが確定する。未割当方向・Spaceのカーソル移動・音声・絵文字の操作は変わらず、製品ページのモックも同じ距離で確定する。
...
- 2026-09-27のユーザ訂正を優先する。前回の10/16/24dpは逆方向だったため、高い/標準/低いの選択・確定開始を16/24/32dpへ伸ばす。高いを旧標準と同じ16dp、標準を旧低いと同じ24dpとし、段階を増やさず低いを32dpとする。中心復帰は9/13/17dp、縦方向専用キーの軸固定は11/15/19dpへ揃える。Spaceのカーソル移動距離・長押し時間・tapの当たり判定は変えない。

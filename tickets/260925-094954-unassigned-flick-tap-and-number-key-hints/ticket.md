...
- [ ] AC 4: 設定画面に「かな・テンキー」と「QWERTY・記号」の独立したフリック感度設定があり、それぞれ「高い」「標準」「低い」を選べる。初期値は両方「標準」で、選択はアプリを終了・再起動しても保持される。
- [ ] AC 5: 「高い」では「標準」より短い移動、「低い」では長い移動で割当済み方向が選択される。かな・数字レイヤーの設定を変えてもQWERTY・記号レイヤーの選択距離は変わらず、逆も同じである。変更は次に開くキーボードに反映される。
- [ ] AC 6: どの感度でも未割当方向は通常タップとして一度だけ入力され、中心復帰・取消・中央長押し・Spaceのカーソル移動は維持される。
- [ ] AC 7: 製品ページの操作モックでも二つの感度を別々に切り替え、同じキーと移動距離で選択の違いを試せる。表示する感度名と初期値はAndroid設定と一致する。

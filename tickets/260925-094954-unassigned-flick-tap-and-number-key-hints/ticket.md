...
- [ ] AC 8: 「かな・テンキー」「QWERTY・記号」の各感度で、v0.15.25より長く指を動かして初めて割当済みフリックが確定する。「高い」は旧「標準」と同程度の距離となり、途中で離したときはタップが確定する。未割当方向・Spaceのカーソル移動・音声・絵文字の操作は変わらず、製品ページのモックも同じ距離で確定する。

# Work Notes: 260911-102026-add-enter-up-flick-ctrl-j
...
- [-] PDH-implement: 外部 provider 経由 path は実 API 200 確認済み (deferred の場合は明示記録) - skip: 外部providerを使わない端末内キーイベント実装
...
- [x] 測る対象を洗い出し、native gesture testとブラウザ実操作でCtrl+J送信・C-j選択表示・下Paste遷移を確認した
...
|---|---|---|---|---|---|
|   |      |     |      |      |      |
| 1 | 配布物 | Major | v0.9 ZIPが未生成 | 採用・修正済み | APK入りZIPを生成し、全リンクと一致を確認 |
...
ユーザの直接指示を承認済み契約として確認。変換中の上フリック既存動作を維持する条件もACへ反映済み。
- `2c23b43`: Enter上フリックへCtrl+Jを追加。待機時ラベルなし、選択時のみC-jを表示。
- `site/mock.html`: nativeと同じ上下フリックと90msのラベル遷移へ同期。
Enter上Ctrl+Jと選択時だけ表示するC-jをdecision 21へ記録した。
APK、操作モック、スクリーンショットと全suite結果を提示する。ユーザのクローズ承認待ち。

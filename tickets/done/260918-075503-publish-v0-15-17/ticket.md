...
## 260918-075503-publish-v0-15-17

### Why
QWERTYの半幅句点・疑問符と全幅Backspaceを使える修正版を、利用者が直接導入できる状態にする。
...
このticketが終わると、利用者がQWERTY右端キーを入れ替えた最新版APKをGitHub Releaseと製品ページから直接取得できる。

- [x] AC 1: GitHubのpublicな`masuidrive/android-kbd` mainにv0.15.17のソースと公開記録があり、Releaseには未圧縮APKが1個だけ添付される。
- [x] AC 2: 公開APKがローカル最終成果物とbyte一致し、package、versionCode 33、versionName 0.15.17、ARM64 ABI、RECORD_AUDIOあり、INTERNETなしを確認できる。
- [x] AC 3: 公開APKと操作モックで、QWERTY 2行目右端が0.5wの`.`／下フリック`?`、3行目右端が1w Backspace／下フリックEscとなり、未割当方向がno-opである。
- [x] AC 4: `https://masuidrive.jp/products/md-kbd/`とマニュアルがv0.15.17のAPK直リンクと変更点を表示し、スマホ・wide幅で横overflowを起こさない。
...
- 版番号は直前のv0.15.16からpatchを1つ上げ、versionCodeは32から33へ上げる。
- GitHub Release assetはZIP化せず`gesture-ime-v0.15.17.apk`だけにする。
...
- 未承認の音声疑似スペクトラムticketをこのReleaseへ含めること。
...
---
Work notes: `note.md`
- 260918-052256-swap-qwerty-period-backspace（main統合済み）
